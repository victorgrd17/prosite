...
contratos_disponiveis = sorted(df['tipo_de_contrato'].unique())
...
df_filtrado = df[
    ...
    (df['tipo_de_contrato'].isin(contratos_selecionados)) &
...
if not df_filtrado.empty:
    ...
    cargo_mais_frequente = df_filtrado["carreira"].mode()[0]
else:
...
with col_graf1:
    if not df_filtrado.empty:
        top_cargos = df_filtrado.groupby('carreira')['dolar'].mean().nlargest(
        ...
        grafico_cargos = px.bar(
            ...
            labels={'dolar': 'Média salarial anual (USD)', 'carreira': ''}
        )
...
with col_graf4:
    if not df_filtrado.empty:
        df_ds = df_filtrado[df_filtrado['carreira'] == 'Data Scientist']
        media_ds_pais = df_ds.groupby('residencia_iso3')[
            'dolar'].mean().reset_index()
